...
func Test(t *testing.T) {
	...
	containerOptions := []testcontainers.ContainerCustomizer{
		...
		testcontainers.WithExposedPorts("3333/tcp", "3334/tcp"),
	}
	...
	containerOptions = append(containerOptions,
		...
		testcontainers.WithWaitStrategyAndDeadline(5*time.Minute,
			wait.ForHTTP("/hc/").WithPort("3333/tcp").WithStartupTimeout(5*time.Minute),
			wait.ForHTTP("/hc/").WithPort("3334/tcp").WithStartupTimeout(5*time.Minute),
		),
	)
